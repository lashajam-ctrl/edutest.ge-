...
import { adminEmailMfa, EmailMfaError } from "@/lib/admin-email-mfa";
...
export async function GET(request: Request) {
  ...
  return noStore({ ...await adminMfaState(current), ...await adminEmailMfa().status(current) });
}
...
export async function POST(request: Request) {
  ...
  try {
    const emailMfa = adminEmailMfa();
    if (body.action === "email_send") return noStore(await emailMfa.send(current));
    if (body.action === "email_verify") {
      const code = typeof body.code === "string" ? body.code : "";
      if (!(await emailMfa.verify(current, code))) return noStore({ error: "კოდი არასწორია, ვადა გაუვიდა ან უკვე გამოყენებულია." }, { status: 400 });
      return noStore({ verified: true });
    }
    if (await emailMfa.status(current)) return noStore({ error: "ამ ანგარიშისთვის გამოიყენეთ ელფოსტაზე მიღებული კოდი." }, { status: 403 });
    ...
    if (error instanceof EmailMfaError) return noStore({ error: error.message }, { status: error.status, headers: error.retryAfter ? { "Retry-After": String(error.retryAfter) } : {} });
