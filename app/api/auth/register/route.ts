...
export async function POST(request: Request) {
  ...
  const now = new Date();
  const user = { id: crypto.randomUUID(), email, name, role: body.role === "teacher" ? "pending_teacher" as const : "student" as const, grade: body.grade || null, school: body.school || null, passwordHash: passwordData.hash, passwordSalt: passwordData.salt, emailVerified: false, accountStatus: "onboarding", createdAt: now, updatedAt: now };
