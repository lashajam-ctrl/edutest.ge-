...
export function ensureSchema() {
  ...
  if (!schemaReady) {
    schemaReady = env.DB.batch([
      ...
      env.DB.prepare("CREATE TABLE IF NOT EXISTS admin_mfa_factors (user_id text PRIMARY KEY NOT NULL REFERENCES users(id) ON DELETE cascade, encrypted_secret text NOT NULL, confirmed_at integer, last_used_counter integer DEFAULT -1 NOT NULL, created_at integer NOT NULL, updated_at integer NOT NULL)"),
      env.DB.prepare("CREATE TABLE IF NOT EXISTS session_mfa_verifications (session_id text PRIMARY KEY NOT NULL REFERENCES sessions(id) ON DELETE cascade, verified_at integer NOT NULL, expires_at integer NOT NULL)"),
      env.DB.prepare("CREATE INDEX IF NOT EXISTS idx_session_mfa_expires ON session_mfa_verifications (expires_at)"),
      ...
      env.DB.prepare("CREATE TABLE IF NOT EXISTS email_verification_requests (id text PRIMARY KEY NOT NULL, user_id text NOT NULL REFERENCES users(id) ON DELETE cascade, token_hash text NOT NULL, expires_at integer NOT NULL, used_at integer, created_at integer NOT NULL)"),
      env.DB.prepare("CREATE UNIQUE INDEX IF NOT EXISTS email_verification_token_unique ON email_verification_requests (token_hash)"),
      env.DB.prepare("CREATE INDEX IF NOT EXISTS idx_email_verification_user_expires ON email_verification_requests (user_id, expires_at)"),
      ...
      env.DB.prepare("CREATE TABLE IF NOT EXISTS user_learning_state (user_id text PRIMARY KEY NOT NULL REFERENCES users(id) ON DELETE cascade, state_json text DEFAULT '{}' NOT NULL, updated_at integer NOT NULL)"),
