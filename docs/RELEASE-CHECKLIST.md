...
3. Record current source SHA and Cloudflare version. Review unapplied migrations; apply only additive migrations. `0011` adds email confirmation tables and `0012` adds isolated learning practice sessions. Do not add duplicate runtime CREATEs for these migration-owned tables. Apply migrations to fresh local databases too.
...
- Human question review is not tracked; source `curriculum_reviewed` labels are not evidence. Bank group totals are not guaranteed daily test capacity.
