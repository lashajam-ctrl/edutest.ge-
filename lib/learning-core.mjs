...
export function makeLearningPlan({ user, tests, history, completed = 0, now = Date.now() }) {
  ...
  const reviewAt = h => Math.max(Number(h.next_review_at)||0, Number(h.remediated_at)>Number(h.last_answered_at||0)?Number(h.remediated_at)+7*DAY_MS:0);
  const reinforced = history.filter(h=>Number(h.last_correct)===0 && Number(h.remediated_at)>Number(h.last_answered_at||0) && reviewAt(h)>now);
  const weak = history.filter(h => Number(h.last_correct) === 0 && !reinforced.includes(h)).sort((a,b) => reviewAt(a)-reviewAt(b) || String(a.question_id).localeCompare(String(b.question_id)));
  const due = history.filter(h => reviewAt(h) <= now).sort((a,b)=>reviewAt(a)-reviewAt(b));
  const actions = [];
  ...
  return { ...day, method: 'server_history_rules', completedToday: completed, answeredQuestions: history.length,
    dueQuestions: due.length, mistakes: weak.length, reinforcedMistakes:reinforced.length, actions,
...
export function weeklySummary(rows, now = Date.now()) {
  ...
  for (const row of rows) {
    ...
    for (const q of result.reviewed) {
      if (q.gradingStatus === 'pending' || typeof q.ok !== 'boolean') continue;
