(function expandEduTestQuestionBank(root){
  ...
  const fillInstruction=text=>{
    ...
    if(/(?:ჩაწერე|ჩასვი|შეავსე|დაასრულე|complete|insert|fill|вставьте|заполните)/iu.test(value))return value;
    if(/[\u10A0-\u10FF]/u.test(value))return `ჩაწერე გამოტოვებული პასუხი: ${value}`;
    if(/[\u0400-\u04FF]/u.test(value))return `Вставьте пропущенный ответ: ${value}`;
    return `Complete the missing part: ${value}`;
  };
  ...
  const meta=(q,prefix,grade,index,skill)=>({
    ...
    skill:`${prefix}.${skill}`,
    templateFamily:q.templateFamily||undefined,
  ...
  const numericDistractors=(correct,min,max)=>{
    const candidates=[correct-1,correct+1,correct-2,correct+2,min,max]
      .filter(value=>Number.isFinite(value)&&value>=min&&value<=max&&value!==correct);
    return [...new Set(candidates)].slice(0,3);
  };
  const primaryContext=i=>{
    const sequence=Math.max(0,Math.floor((Number(i)-1)/2));
    return{
      semester:sequence%2+1,
      family:Math.floor(sequence/2)%60,
      variant:Math.floor(sequence/120)
    };
  };
  const primaryMeta=(q,prefix,grade,i,skill,context)=>{
    if(context.semester===2)q.text=`ყურადღებით წაიკითხე და შეასრულე: ${q.text}`;
    if(q.visual)q.visual={...q.visual,variantKey:`g${grade}-s${context.semester}-v${context.variant}-f${context.family}`};
    q.semester=context.semester;
    q.templateFamily=`${prefix}.g${grade}.${skill}.f${context.family}`;
    // Same learning operation with changed names/numbers is one session shape.
    // The selector caps these shapes so a 20-question paper stays varied.
    q.templateShape=`${prefix}.g${grade}.${skill}`;
    return meta(q,prefix,grade,i,skill);
  };

  function mathPrimaryQuestion(grade,i){
    const context=primaryContext(i),slot=context.family%20,theme=Math.floor(context.family/20);
    const settings=[
      ['ნინას','ფანქარი','საკალმეში'],['ლუკას','ვაშლი','კალათაში'],
      ['ანას','წიგნი','თაროზე'],['საბას','ბურთი','ყუთში'],['თეკლას','სტიკერი','რვეულში']
    ];
    const [owner,item,place]=settings[theme];
    const limit=grade===1?20:100;
    const seed=grade*10000+context.variant*211+context.semester*47+context.family*13;
    const a=grade===1?n(seed,3,10):n(seed,12,48);
    const b=grade===1?n(seed+7,1,Math.max(2,limit-a)):n(seed+7,3,Math.max(4,limit-a));
    const total=Math.min(limit,a+b);
    const small=Math.min(a,total-1);
    let q,skill;
    switch(slot){
      case 0:
        skill='addition_fluency';q=calcQ(`გამოთვალე: ${a} + ${b} = ?`,total,`${a}-სა და ${b}-ის ჯამია ${total}.`);break;
      case 1:
        skill='subtraction_fluency';q=calcQ(`გამოთვალე: ${total} − ${b} = ?`,total-b,`${total}-დან ${b}-ის გამოკლებით მივიღებთ ${total-b}.`);break;
      case 2:
        skill='one_step_addition_story';q=mcq(`${owner} ჰქონდა ${a} ${item}. კიდევ ${b} ${item} მისცეს. რამდენი ${item} აქვს ახლა?`,total,numericDistractors(total,0,limit),`ერთი შეკრებაა საჭირო: ${a}+${b}=${total}.`);break;
      case 3:
        skill='one_step_subtraction_story';q=mcq(`${place} იდო ${total} ${item}. ${b} აიღეს. რამდენი ${item} დარჩა?`,total-b,numericDistractors(total-b,0,limit),`ერთი გამოკლებაა საჭირო: ${total}−${b}=${total-b}.`);break;
      case 4:{
        skill='number_order';const values=[small,Math.min(limit,small+2),Math.min(limit,small+4)];
        q=orderQ('დაალაგე რიცხვები უმცირესიდან უდიდესამდე.',[...new Set(values)].sort((x,y)=>x-y).map(String),'რიცხვები ზრდადობით იწყება უმცირესით.');break;
      }
      case 5:{
        skill='number_pattern';const step=grade===1?1:pick([2,5,10],seed),start=grade===1?n(seed,1,12):n(seed,5,45);
        q=fillQ(`ჩაწერე გამოტოვებული რიცხვი: ${start}, ${start+step}, ___, ${start+3*step}.`,[start+2*step],`ყოველ ჯერზე ემატება ${step}.`);break;
      }
      case 6:
        skill='missing_addend';q=fillQ(`ჩაწერე გამოტოვებული რიცხვი: ${small} + ___ = ${total}.`,[total-small],`${total}-${small}=${total-small}.`);break;
      case 7:{
        skill='number_comparison';const other=Math.max(0,small-1);
        q=mcq(`რომელი რიცხვია უფრო დიდი: ${small} თუ ${other}?`,small,[other],`${small} მეტია ${other}-ზე.`);break;
      }
      case 8:{
        skill='number_between';const middle=Math.min(limit-1,Math.max(2,small));
        q=mcq(`რომელი რიცხვია ${middle-1}-სა და ${middle+1}-ს შორის?`,middle,numericDistractors(middle,0,limit),`${middle-1}<${middle}<${middle+1}.`);break;
      }
      case 9:{
        skill='visual_data_comparison';const values=[small,Math.min(limit,small+1),Math.max(0,small-2)],labels=['A','B','C'],max=Math.max(...values),answer=labels[values.indexOf(max)];
        q=mcq('დიაგრამაზე რომელი სვეტია ყველაზე მაღალი?',answer,labels.filter(label=>label!==answer),`${answer} სვეტის მნიშვნელობაა ${max} და ის უდიდესია.`);
        q.visual=visual('bars',labels.map((label,index)=>`${label}: ${values[index]}`).join(', '),{labels,values},'შეადარე სამი სვეტის სიმაღლე');break;
      }
      case 10:{
        skill='operation_match';const x=Math.max(1,small-2);
        q=matchQ('დააკავშირე თითოეული მოქმედება სწორ პასუხთან.',[[`${x}+1`,String(x+1)],[`${x}+2`,String(x+2)],[`${x}+3`,String(x+3)]],'თითოეული ჯამი ცალ-ცალკე გამოთვალე.');break;
      }
      case 11:
        skill='equality_check';q=tf(`სწორია თუ მცდარია: ${a} + ${b} = ${total}.`,true,`${a}+${b}=${total}.`);break;
      case 12:{
        skill='predecessor';const value=Math.max(2,small);
        q=mcq(`რომელი რიცხვია ${value}-ის წინა რიცხვი?`,value-1,numericDistractors(value-1,0,limit),`${value}-ის წინა რიცხვია ${value-1}.`);break;
      }
      case 13:{
        skill='successor';const value=Math.min(limit-1,Math.max(1,small));
        q=mcq(`რომელი რიცხვია ${value}-ის მომდევნო რიცხვი?`,value+1,numericDistractors(value+1,0,limit),`${value}-ის მომდევნო რიცხვია ${value+1}.`);break;
      }
      case 14:{
        skill='expression_comparison';const lower=Math.max(0,b-1),left=`${a} + ${b}`,right=`${a} + ${lower}`;
      ...
      case 16:{
        ...
        q=mcq(`რამდენი ათეული და ერთეულია რიცხვ ${value}-ში?`,`${tens} ათეული და ${ones} ერთეული`,[
          `${ones} ათეული და ${tens} ერთეული`,
          `${tens} ათეული და ${Math.min(9,ones+1)} ერთეული`
        ],`${value}=${tens}×10+${ones}.`);break;
      }
      case 17:{
        skill='visual_counting';const count=Math.min(10,Math.max(3,small));
        q=mcq('რამდენი წრეა გამოსახული?',count,numericDistractors(count,0,limit),`სულ ${count} წრეა.`);
        q.visual=visual('tokens',`${count} მწვანე წრე`,{items:Array.from({length:count},()=> '●')},'დათვალე წრეები');break;
      }
      case 18:{
        skill='shape_properties';const shapes=[['სამკუთხედს','△',3],['კვადრატს','□',4],['წრეს','○',0]],chosen=shapes[seed%shapes.length];
        q=mcq(`${chosen[0]} რამდენი გვერდი აქვს?`,chosen[2],[0,3,4].filter(value=>value!==chosen[2]),`${chosen[0]} ${chosen[2]} გვერდი აქვს.`);
        q.visual=visual('cards','სამკუთხედი, კვადრატი და წრე',{items:shapes.map(row=>row[1])},'დააკვირდი გეომეტრიულ ფიგურებს');break;
      }
      default:
        skill='addition_order_property';q=tf(`სწორია თუ მცდარია: ${a} + ${b} = ${b} + ${a}.`,true,'შესაკრებთა ადგილების შეცვლით ჯამი არ იცვლება.');break;
    }
    return primaryMeta(q,'math',grade,i,skill,context);
  }

  function georgianEarlyQuestion(grade,i){
    const context=primaryContext(i),slot=context.family%12,theme=Math.floor(context.family/12);
    const rows=[
      ['ანა','ხატავს','სურათს','მზე','☀️',1],
      ['ლუკა','კითხულობს','წიგნს','დედა','👩',2],
      ['ნინო','რწყავს','ყვავილს','სკოლა','🏫',2],
      ['საბა','აგორებს','ბურთს','კატა','🐈',2],
      ['თეკლა','ავსებს','რვეულს','თევზი','🐟',2],
      ['მარი','ხსნის','კარს','ხე','🌳',1],
      ['დათა','უყურებს','ძაღლს','ძაღლი','🐕',2],
      ['ლიზი','ჭამს','მსხალს','ბურთი','⚽',2],
      ['გიო','წერს','ასოს','სახლი','🏠',2],
      ['ელენე','უსმენს','ამბავს','ყვავილი','🌸',3]
    ];
    const rowIndex=(theme+(context.semester-1)*5+context.variant*10)%rows.length;
    const [name,action,object,word,emoji,syllables]=rows[rowIndex];
    const letters=[...word],first=letters[0],last=letters[letters.length-1];
    let q,skill;
    switch(slot){
      case 0:
        skill='letter_recognition';q=mcq(`რომელი ასოთი იწყება სიტყვა „${word}“?`,first,[last,'ბ','რ'],`სიტყვა „${word}“ იწყება ასოთი „${first}“.`);break;
      case 1:
        skill='ending_letter';q=mcq(`რომელი ასოთი მთავრდება სიტყვა „${word}“?`,last,[first,'ა','მ'],`სიტყვა „${word}“ მთავრდება ასოთი „${last}“.`);break;
      case 2:
        skill='syllable_count';q=mcq(`რამდენი მარცვალია სიტყვაში „${word}“?`,syllables,numericDistractors(syllables,1,4),`სიტყვა „${word}“ იყოფა ${syllables} მარცვლად.`);break;
      case 3:
        skill='picture_word';q=mcq('რომელი სიტყვა შეესაბამება სურათს?',word,['ხე','სახლი','წიგნი'].filter(value=>value!==word),`სურათზე გამოსახულია ${word}.`);q.visual=visual('scene',`${word} ${emoji}`,{items:[emoji],label:word},'დააკვირდი სურათს');break;
      case 4:
        skill='action_word';q=mcq(`წაიკითხე: „${name} ${action} ${object}.“ რომელი სიტყვა გვიჩვენებს მოქმედებას?`,action,[name,object.replace(/ს$/,''),'დღეს'],`მოქმედების სიტყვაა „${action}“.`);break;
      case 5:
        skill='sentence_order';q=orderQ('დაალაგე სიტყვები ისე, რომ სწორი წინადადება მიიღო.',[name,action,object+'.'],`სწორი წინადადებაა: ${name} ${action} ${object}.`);break;
      case 6:
        skill='sentence_completion';q=fillQ(`ჩასვი გამოტოვებული სიტყვა: ${name} ___ ${object}.`,[action],`წინადადებას ასრულებს სიტყვა „${action}“.`);break;
      case 7:
        skill='question_mark';q=mcq(`აირჩიე ნიშანი, რომელიც უნდა დაიწეროს წინადადების ბოლოს: „სად არის ${word}“`,'?',['.','!'],`ეს კითხვაა, ამიტომ ბოლოს იწერება კითხვის ნიშანი.`);break;
      case 8:
        skill='capital_letter';q=tf(`სწორია თუ მცდარია: სახელი „${name}“ დიდი ასოთი უნდა დაიწყოს.`,true,'ადამიანის სახელი დიდი ასოთი იწერება.');break;
      case 9:
        skill='reading_detail';q=mcq(`${name} ეზოში გავიდა და ${word} დაინახა. რა დაინახა ${name}-მ?`,word,['მანქანა','მდინარე','თოვლი'].filter(value=>value!==word),`ტექსტში პირდაპირ წერია, რომ ${name}-მ ${word} დაინახა.`);break;
      case 10:
        skill='word_match';q=matchQ(`დააკავშირე სიტყვები შესაბამის გამოსახულებებთან — დაიწყე სიტყვით „${word}“.`,[[word,`გამოსახულება: ${word}`],['კატა','გამოსახულება: კატა'],['თევზი','გამოსახულება: თევზი']].filter((row,index,all)=>all.findIndex(other=>other[0]===row[0])===index),'თითოეულ სიტყვას თავისი გამოსახულება შეესაბამება.');break;
      default:
        skill='sentence_meaning';q=mcq(`წაიკითხე: „${name} ${action} ${object}.“ ვინ ასრულებს მოქმედებას?`,name,[word,'მასწავლებელი','მეგობარი'].filter(value=>value!==name),`მოქმედებას ასრულებს ${name}.`);break;
    }
    return primaryMeta(q,'geo',grade,i,skill,context);
  }

  function englishEarlyQuestion(grade,i){
    const context=primaryContext(i),slot=context.family%12,theme=Math.floor(context.family/12);
    const vocab=[
      ['cat','კატა','🐈'],['book','წიგნი','📘'],['sun','მზე','☀️'],
      ['apple','ვაშლი','🍎'],['bird','ჩიტი','🐦'],['dog','ძაღლი','🐕'],
      ['tree','ხე','🌳'],['ball','ბურთი','⚽'],['fish','თევზი','🐟'],['flower','ყვავილი','🌸']
    ];
    const rowIndex=(theme+(context.semester-1)*5+context.variant*10)%vocab.length;
    const [word,ka,emoji]=vocab[rowIndex],article=/^[aeiou]/i.test(word)?'an':'a';
    const alternatives=vocab.filter(row=>row[0]!==word).map(row=>row[0]).slice(0,3);
    let q,skill;
    switch(slot){
      case 0:
        skill='bilingual_vocabulary';q=mcq(`აირჩიე სწორი ინგლისური სიტყვა: „${ka}“`,word,alternatives,`„${ka}“ ინგლისურად არის “${word}”.`);break;
      case 1:
        skill='bilingual_meaning';q=mcq(`რას ნიშნავს ინგლისური სიტყვა “${word}”?`,ka,vocab.filter(row=>row[1]!==ka).map(row=>row[1]).slice(0,3),`“${word}” ნიშნავს „${ka}“-ს.`);break;
      case 2:
        skill='visual_vocabulary';q=mcq('დააკვირდი სურათს და აირჩიე სწორი ინგლისური სიტყვა.',word,alternatives,`${emoji} ინგლისურად არის “${word}”.`);q.visual=visual('scene',`${ka}: ${emoji}`,{items:[emoji],label:ka},'Look and choose / შეხედე და აირჩიე');break;
      case 3:
        skill='guided_word_completion';q=fillQ(`ჩასვი ინგლისური სიტყვა: This is ${article} ___. (${ka})`,[word],`გამოტოვებული სიტყვაა “${word}”.`);break;
      case 4:
        skill='greeting';{
          const greetings=[
            ['დილით მეგობარს როგორ მივესალმოთ ინგლისურად?','Good morning',['Good night','Goodbye','Thank you']],
            ['მეგობარს ჩვეულებრივ როგორ მივესალმოთ ინგლისურად?','Hello',['Goodbye','Thank you','Good night']],
            ['დამშვიდობებისას რომელი ინგლისური სიტყვა გამოვიყენოთ?','Goodbye',['Hello','Please','Good morning']],
            ['მადლობის სათქმელად რომელი ინგლისური სიტყვაა საჭირო?','Thank you',['Goodbye','Hello','Good night']],
            ['ძილის წინ რომელი ინგლისური ფრაზაა შესაფერისი?','Good night',['Good morning','Hello','Thank you']]
          ],row=greetings[theme];
          q=mcq(row[0],row[1],row[2],`სწორი ფრაზაა “${row[1]}”.`);break;
        }
      case 5:{
        skill='color_word';const colors=[['red','წითელი','🔴'],['blue','ლურჯი','🔵'],['green','მწვანე','🟢'],['yellow','ყვითელი','🟡'],['black','შავი','⚫']],[en,ge,symbol]=colors[theme];
        q=mcq(`რომელი ინგლისური სიტყვა აღნიშნავს ფერს ${symbol} „${ge}“?`,en,colors.filter(row=>row[0]!==en).map(row=>row[0]).slice(0,3),`${ge} ინგლისურად არის “${en}”.`);break;
      }
      case 6:{
        skill='number_word';const value=theme+1,words=['one','two','three','four','five'];
        q=mcq(`აირჩიე რიცხვი ${value}-ის ინგლისური სახელი.`,words[theme],words.filter(value=>value!==words[theme]).slice(0,3),`${value} ინგლისურად არის “${words[theme]}”.`);break;
      }
      case 7:
        skill='vocabulary_match';{
          const rotated=[...vocab.slice(rowIndex),...vocab.slice(0,rowIndex)].slice(0,3);
          q=matchQ(`დააკავშირე ინგლისური სიტყვები ქართულ მნიშვნელობებთან — დაიწყე “${word}”-ით.`,rotated.map(row=>[row[0],row[1]]),'თითოეული ინგლისური სიტყვა შესაბამის ქართულ მნიშვნელობას დაუკავშირე.');break;
        }
      case 8:
        skill='simple_sentence_order';q=orderQ(`დაალაგე სიტყვები და ააწყვე მარტივი ინგლისური წინადადება სიტყვით “${word}”.`,['I','see',article,word+'.'],`სწორი წინადადებაა: I see ${article} ${word}.`);break;
      case 9:
        skill='translation_check';q=tf(`სწორია თუ მცდარია: “${word}” ნიშნავს „${ka}“-ს.`,true,`“${word}” ნიშნავს „${ka}“-ს.`);break;
      case 10:
        skill='classroom_word';q=mcq(`აირჩიე ინგლისური სიტყვა, რომელსაც გაკვეთილზე გამოიყენებ „${ka}“-სთვის.`,word,alternatives,`სწორი სიტყვაა “${word}”.`);break;
      default:
        skill='word_in_sentence';q=mcq(`დააკვირდი სურათს და შეავსე წინადადება: “I see ${article} ___.”`,word,alternatives,`სურათზეა ${ka}; სწორი სიტყვაა “${word}”.`);q.visual=visual('scene',`${ka}: ${emoji}`,{items:[emoji],label:ka},'Look and complete / შეხედე და შეავსე');break;
    }
    q.bilingual=true;
    return primaryMeta(q,'eng',grade,i,skill,context);
  }

  function natureEarlyQuestion(grade,i){
    const context=primaryContext(i),slot=context.family%12,theme=Math.floor(context.family/12);
    const animals=[
      ['ძაღლი','🐕','ცხოველი','სახლი'],['ბეღურა','🐦','ფრინველი','ბუდე'],
      ['თევზი','🐟','ცხოველი','წყალი'],['პეპელა','🦋','მწერი','ბაღი'],['კატა','🐈','ცხოველი','სახლი'],
      ['ძროხა','🐄','ცხოველი','მინდორი'],['ქათამი','🐔','ფრინველი','საქათმე'],
      ['კურდღელი','🐇','ცხოველი','სორო'],['ფუტკარი','🐝','მწერი','სკა'],['ბაყაყი','🐸','ცხოველი','ტბა']
    ];
    const rowIndex=(theme+(context.semester-1)*5+context.variant*10)%animals.length;
    const [name,emoji,group,habitat]=animals[rowIndex];
    let q,skill;
    switch(slot){
      case 0:
        skill='animal_identification';q=mcq('რომელი ცხოველია გამოსახული?',name,animals.filter(row=>row[0]!==name).map(row=>row[0]).slice(0,3),`სურათზე გამოსახულია ${name}.`);q.visual=visual('scene',`${name} ${emoji}`,{items:[emoji],label:name},'დააკვირდი ცხოველს');break;
      case 1:
        skill='bird_or_animal';q=mcq(`${name} არის ფრინველი თუ სხვა ცხოველი?`,group==='ფრინველი'?'ფრინველი':'სხვა ცხოველი',[group==='ფრინველი'?'სხვა ცხოველი':'ფრინველი'],`${name} არის ${group}.`);q.visual=visual('scene',`${name} ${emoji}`,{items:[emoji],label:name},'დააკვირდი გამოსახულებას');break;
      case 2:
        skill='living_nonliving';q=mcq(`რომელია ცოცხალი: ${name} თუ ქვა?`,name,['ქვა'],`${name} იზრდება, იკვებება და სუნთქავს.`);break;
      case 3:{
        skill='plant_parts';const parts=[['ფესვი','ნიადაგიდან წყალს იღებს'],['ღერო','მცენარეს ამაგრებს'],['ფოთოლი','სინათლეს იღებს'],['ყვავილი','ნაყოფის წარმოქმნაში მონაწილეობს'],['ნაყოფი','თესლს იცავს']],[part,use]=parts[theme];
        q=mcq(`მცენარის რომელი ნაწილია აღწერილი: „${use}“?`,part,parts.filter(row=>row[0]!==part).map(row=>row[0]).slice(0,3),`ეს არის ${part}.`);
        q.visual=visual('tokens',`მცენარის ნაწილი: ${part}`,{items:['🌱',part]},'დააკვირდი მცენარის ნაწილს');break;
      }
      case 4:{
        skill='seasons';const seasons=[['გაზაფხული','ყვავილები იშლება'],['ზაფხული','ხშირად ცხელა'],['შემოდგომა','ფოთლები ცვივა'],['ზამთარი','ხშირად თოვს'],['გაზაფხული','ბუნება იღვიძებს']],[season,sign]=seasons[theme];
        q=mcq(`რომელ სეზონს ახასიათებს: „${sign}“?`,season,['გაზაფხული','ზაფხული','შემოდგომა','ზამთარი'].filter(value=>value!==season).slice(0,3),`${sign} — ეს ${season}ს ახასიათებს.`);
        q.visual=visual('scene',`${season}: ${sign}`,{items:[pick(['🌷','☀️','🍂','❄️','🌱'],theme)],label:season},'დააკვირდი სეზონის ნიშანს');break;
      }
      case 5:{
        skill='five_senses';const senses=[['თვალი','ვხედავთ'],['ყური','ვუსმენთ'],['ცხვირი','სუნს ვგრძნობთ'],['ენა','გემოს ვგრძნობთ'],['კანი','შეხებას ვგრძნობთ']],[organ,action]=senses[theme];
        q=mcq(`რომელი ორგანოთი ${action}?`,organ,senses.filter(row=>row[0]!==organ).map(row=>row[0]).slice(0,3),`${action} ${organ}თი.`);
        q.visual=visual('tokens',`გრძნობის ორგანო: ${organ}`,{items:[pick(['👁️','👂','👃','👅','✋'],theme)],label:organ},'დააკვირდი გრძნობის ორგანოს');break;
      }
      case 6:
        skill='healthy_habit';{
          const habits=[
            ['ჭამის წინ რომელი ქმედებაა ჯანმრთელობისთვის სასარგებლო?','ხელების დაბანა',['ბინძური ხელით ჭამა','ხელების არდაბანა']],
            ['რომელი სასმელია ყოველდღიურად სასარგებლო არჩევანი?','წყალი',['მხოლოდ ტკბილი გაზიანი სასმელი','უცნობი სითხე']],
            ['ღამით ორგანიზმის დასვენებას რა ეხმარება?','საკმარისი ძილი',['მთელი ღამე ფხიზლად ყოფნა','ძილის გამოტოვება']],
            ['კბილების მოვლისთვის რომელი ქმედებაა სწორი?','კბილების გახეხვა',['ტკბილეულის შემდეგ კბილების არგაწმენდა','ჯაგრისის არგამოყენება']],
            ['სხეულის გაძლიერებას რომელი ქმედება ეხმარება?','მოძრაობა და თამაში',['მთელი დღე უმოძრაოდ ყოფნა','სუფთა ჰაერზე არგასვლა']]
          ],row=habits[theme];
          q=mcq(row[0],row[1],row[2],`${row[1]} ჯანმრთელობისთვის სასარგებლოა.`);break;
        }
      case 7:
        skill='environment_care';{
          const actions=[
            ['ქუჩის სისუფთავეს რომელი ქმედება ეხმარება?','ნარჩენის ურნაში ჩაგდება',['ნარჩენის ქუჩაში დაყრა','ქაღალდის ძირს დაგდება']],
            ['წყლის დაზოგვას რომელი ქმედება ეხმარება?','ონკანის დაკეტვა',['ონკანის ღიად დატოვება','წყლის უმიზნოდ დაღვრა']],
            ['მცენარეების დაცვას რომელი ქმედება ეხმარება?','ყვავილების მოვლა',['ტოტების დამტვრევა','ყვავილების გათელვა']],
            ['ეზოს სისუფთავეს რომელი ქმედება ეხმარება?','ნარჩენის შეგროვება',['ნარჩენის გაფანტვა','პლასტმასის ბალახში დატოვება']],
            ['ქაღალდის დაზოგვას რომელი ქმედება ეხმარება?','ფურცლის ორივე მხარის გამოყენება',['სუფთა ფურცლის გადაგდება','რვეულის უმიზეზოდ დახევა']]
          ],row=actions[theme];
          q=mcq(row[0],row[1],row[2],`${row[1]} გარემოს დაცვას ეხმარება.`);break;
        }
      case 8:
        skill='simple_habitat';q=mcq(`სად შეიძლება ცხოვრობდეს ${name}?`,habitat,['მთვარე','ღრუბელი','რვეული'].filter(value=>value!==habitat),`${name} შეიძლება ცხოვრობდეს გარემოში: ${habitat}.`);break;
      case 9:
        skill='day_night';{
          const observations=[
            ['როდის ჩანს მზე ჩვეულებრივ — დღისით თუ ღამით?','დღისით',['ღამით']],
            ['როდის არის ცა ჩვეულებრივ უფრო ნათელი?','დღისით',['ღამით']],
            ['როდის გვჭირდება ქუჩაში ხშირად ხელოვნური განათება?','ღამით',['დღისით']],
            ['როდის იძინებს ადამიანების უმეტესობა?','ღამით',['შუადღით']],
            ['როდის იწყება დილა — ღამის შემდეგ თუ შუადღის შემდეგ?','ღამის შემდეგ',['შუადღის შემდეგ']]
          ],row=observations[theme];
          q=mcq(row[0],row[1],row[2],`სწორი პასუხია „${row[1]}“.`);break;
        }
      case 10:
        skill='nature_match';q=matchQ(`დააკავშირე არსებები საცხოვრებელ გარემოსთან — დაიწყე „${name}“-ით.`,[[name,habitat],['თევზი','წყალი'],['ბეღურა','ბუდე']].filter((row,index,all)=>all.findIndex(other=>other[0]===row[0])===index),'თითოეულ არსებას მისთვის შესაფერისი გარემო დაუკავშირე.');break;
      default:
        skill='observation_check';q=tf(`სწორია თუ მცდარია: ${name} ცოცხალი არსებაა.`,true,`${name} ცოცხალი არსებაა.`);break;
    }
    return primaryMeta(q,'nat',grade,i,skill,context);
  }

  function mathQuestion(grade,i){
    if(grade<=2)return mathPrimaryQuestion(grade,i);
  ...
  function georgianPrimary(grade,i){
    if(grade<=2)return georgianEarlyQuestion(grade,i);
  ...
  function englishPrimary(grade,i){
    if(grade<=2)return englishEarlyQuestion(grade,i);
  ...
  function natureQuestion(grade,i){
    if(grade<=2)return natureEarlyQuestion(grade,i);
  ...
  function fallbackQuestion(prefix,grade,i){
    ...
    // Primary grades never fall back to the generic upper-grade templates.
    // Advancing by 240 keeps the same reviewed concept family but creates a
    // fresh, grade-bounded instance for the appropriate semester.
    if(grade<=2){
      if(prefix==='math')return mathPrimaryQuestion(grade,i+240);
      if(prefix==='geo')return georgianEarlyQuestion(grade,i+240);
      if(prefix==='eng')return englishEarlyQuestion(grade,i+240);
      if(prefix==='nat')return natureEarlyQuestion(grade,i+240);
    }
    if(prefix==='math'){
