(function(){
  ...
  window.refreshSavedAssessments=async function(){
    ...
    try{const r=await fetch('/api/assessments/draft',{credentials:'include',cache:'no-store'}),data=await r.json();if(identity!==owner())return;if(!r.ok)throw new Error('load');box.replaceChildren();
      window.EDUTEST_SAVED_ASSESSMENTS=Array.isArray(data.sessions)?data.sessions:[];window.dispatchEvent(new CustomEvent('edutest-saved-assessments',{detail:window.EDUTEST_SAVED_ASSESSMENTS}));
      for(const item of window.EDUTEST_SAVED_ASSESSMENTS){const title=document.createElement('h3');title.textContent='გააგრძელე: '+item.test.title;const text=document.createElement('p');text.textContent='კითხვები და შენახული პასუხები შენარჩუნებულია. დრო გვერდის დახურვით არ ჩერდება.';const b=document.createElement('button');b.className='btn btn-primary';b.textContent=Date.now()>item.deadlineAt?'შენახული პასუხების გაგზავნა':'ტესტის გაგრძელება';b.onclick=()=>resumeSavedAssessment(item.sessionId);box.append(title,text,b);}
