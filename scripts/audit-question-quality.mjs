...
function pedagogicalFlags(question) {
  ...
  if (Array.isArray(question.opts) && question.opts.some(option => /^(?:არ ვიცი|არცერთი პასუხი|ყველა პასუხი სწორია|ყველა ზემოთ ჩამოთვლილი|ყველაფერია|სხვა)$/iu.test(normalize(option)))) {
...
const subjectFamily = subject => SUBJECT_FAMILY[subject] ?? subject;
...
const publishedBasePools = new Set(ALL_TESTS
  .filter(isCurriculumEligibleTest)
  .map(test => test.pool));
...
const generatedRows = questionRows.filter(question => question.generated);
const generatedPlaceholderOptions = generatedRows.filter(question => questionPolicy.hasPlaceholderOption(question));
const generatedIncompletePrompts = generatedRows.filter(question => !questionPolicy.hasCompleteInstruction(question));
const generatedGradePolicyViolations = generatedRows.filter(question => !questionPolicy.isGradeAppropriate(question, {
  grade: Number(question.grade),
  subject: question.basePool,
}));
const primarySemanticFamilyChecks = Object.values(Object.groupBy(
  generatedRows.filter(question => Number(question.grade) <= 2 && ["math", "geo", "eng", "nat"].some(prefix => question.basePool.startsWith(`${prefix}-`))),
  question => `${question.basePool}|g${question.grade}|s${question.semester}`,
)).map(rows => ({
  pool: rows[0].basePool,
  grade: Number(rows[0].grade),
  semester: Number(rows[0].semester),
  questions: rows.length,
  semanticFamilies: new Set(rows.map(question => questionPolicy.semanticFingerprint(question))).size,
})).sort((a, b) => `${a.pool}-${a.grade}-${a.semester}`.localeCompare(`${b.pool}-${b.grade}-${b.semester}`, "en"));
const primarySemanticFamilyShortfalls = primarySemanticFamilyChecks.filter(row => row.semanticFamilies < 20);

const eligibleTests = ALL_TESTS.filter(isCurriculumEligibleTest);
const testChecks = eligibleTests.map(test => {
  ...
  if (Number(test.grade) <= 4 && exactGradeTagged.length < Number(test.count || 0)) risks.push("insufficient_exact_grade_primary_questions");
  if (!subjectsForGrade(test.grade).includes(subjectFamily(test.subject))) risks.push("subject_not_available_for_grade");
  return {
    id: test.id,
    title: test.title,
...
const bySubjectGrade = Object.values(Object.groupBy(testChecks, test => `${test.subject} · ${test.grade}`)).map(rows => ({
  ...
  primaryTestsWithInsufficientExactGradeQuestions: rows.filter(row => row.risks.includes("insufficient_exact_grade_primary_questions")).length,
}));
...
const report = {
  ...
  summary: {
    ...
    primaryTestsWithInsufficientExactGradeQuestions: testChecks.filter(row => row.risks.includes("insufficient_exact_grade_primary_questions")).length,
    testsOutsideGradeSubjectMap: ALL_TESTS.filter(test => !isCurriculumEligibleTest(test)).length,
    generatedPlaceholderOptions: generatedPlaceholderOptions.length,
    generatedIncompletePrompts: generatedIncompletePrompts.length,
  ...
  generatedPlaceholderOptionExamples: generatedPlaceholderOptions.slice(0, 100).map(question => ({ id: question.id, poolKey: question.poolKey, text: question.text })),
  generatedIncompletePromptExamples: generatedIncompletePrompts.slice(0, 100).map(question => ({ id: question.id, poolKey: question.poolKey, text: question.text })),
  generatedGradePolicyViolationExamples: generatedGradePolicyViolations.slice(0, 100).map(question => ({ id: question.id, poolKey: question.poolKey, grade: question.grade, text: question.text })),
  primarySemanticFamilyChecks,
  primarySemanticFamilyShortfalls,
  limitations: [
...
| I–IV კლასის ტესტი ზუსტი კლასის საკმარისი კითხვების გარეშე | ${report.summary.primaryTestsWithInsufficientExactGradeQuestions} |
| მოსწავლის კატალოგიდან კლასის/საგნის წესით დამალული ძველი ტესტი | ${report.summary.testsOutsideGradeSubjectMap} |
| ახალ კითხვებში ხელოვნური „ვარიანტი N“ პასუხი | ${report.summary.generatedPlaceholderOptions} |
| ახალ კითხვებში არასრული პირობა | ${report.summary.generatedIncompletePrompts} |
| ახალ კითხვებში კლასის სირთულის წესის დარღვევა | ${report.summary.generatedGradePolicyViolations} |
| I–II კლასის აუზი 20-ზე ნაკლები შინაარსობრივი ოჯახით | ${report.summary.primarySemanticFamilyShortfalls} |
...
if (report.summary.testsWithInsufficientSafeQuestions
  || report.summary.primaryTestsWithInsufficientExactGradeQuestions
