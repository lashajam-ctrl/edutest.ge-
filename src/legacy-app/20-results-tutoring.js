function buildReviewHTML(reviewed){
  return reviewed.map((q,i)=>{
    ...
    const pending=q.gradingStatus==='pending';
    const earned=Number(q.awardedPoints!==undefined?q.awardedPoints:(q.ok?q.pts:0));
    const partial=!pending&&!q.ok&&earned>0;
    const col=(pending||partial)?'fffbeb':q.ok?'f0fdf4':'fff1f2';
    const bc2=(pending||partial)?'fde68a':q.ok?'bbf7d0':'fecaca';
    const pts=pending?t('pending_pts'):q.ok?'✓ '+earned+'/'+q.pts+' '+t('pts_label'):(earned>0?'◐ '+earned+'/'+q.pts+' '+t('pts_label'):'✗ 0/'+q.pts+' '+t('pts_label'));
    const ptsCol=(pending||partial)?'#d97706':q.ok?'#16a34a':'#dc2626';
    ...
    const explainDiv=(q.ok&&q.explain&&q.type!=='short_answer')?
      `<div style="margin-top:5px;font-size:11px;color:#6b7280">💡 ${esc(q.explain)}</div>`:'';
    const shortFeedbackDiv=(q.type==='short_answer'&&q.explain)?
      `<div style="margin-top:6px;padding:8px 10px;background:${pending?'#fffbeb':'#eef6ff'};border-radius:6px;font-size:11px;color:${pending?'#92400e':'#1e3a5f'}">${pending?'⏳ ':'💡 '}${esc(q.explain)}</div>`:'';
    ...
    return `<div style="padding:10px;background:#${col};border:1px solid #${bc2};border-radius:8px;margin-bottom:6px">
      ...
      ${wrongDiv}${explainDiv}${shortFeedbackDiv}${hiddenDiv}${aiDiv}
    </div>`;
  }).join('');
}
