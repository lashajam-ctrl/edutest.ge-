...
function learningResultFingerprint(row){
  if(!row||typeof row!=='object')return '';
  const reviewed=Array.isArray(row.reviewed)?row.reviewed.map(item=>String(item&&item.id||'')).join('|'):'';
  return JSON.stringify([String(row.userId||''),String(row.testId||''),String(row.date||''),Number(row.earned||0),Number(row.totalPts||0),Number(row.correct||0),Number(row.total||0),Number(row.pct||0),reviewed]);
}
async function hydrateServerLearningState(appUser,generation){
  ...
  try{
    ...
    if(attemptPayload){
      ...
      const remoteIds=new Set(remote.map(row=>row._serverAttemptId)),remoteFingerprints=new Set(remote.map(learningResultFingerprint)),seen=new Set();
      const local=[...SESSION_RESULTS,...(_ls('edutest_results')||[])].filter(row=>{
        if(row.userId!==appUser.email||(row._serverAttemptId&&remoteIds.has(row._serverAttemptId))||remoteFingerprints.has(learningResultFingerprint(row)))return false;
